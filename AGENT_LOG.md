...
**Public-push readiness pass (2026-05-05)**
Claude Code · Replaced the upstream Scaffold-ETH 2 `README.md` with a PrizeLock-specific one: high-level pitch, "local/testnet MVP — not production" framing, what is fake vs. real, quickstart commands (`yarn install` / `yarn chain` / `yarn deploy` / `yarn start` / `yarn ci`), file layout, and links to `DEMO.md`, `SPEC.md`, `AGENTS.md`, `CLAUDE.md`, `HANDOFF.md`, `BASE_SEPOLIA.md`, `PRE_TESTNET_CHECKLIST.md`, `SECURITY.md`, `AGENT_LOG.md`. Hardened root `.gitignore` defensively: `.env`, `.env.*` (with `!.env.example` + `!**/.env.example` allowlist), `*.pem`, `*.key`, `secrets.json`. Confirmed no real `.env` or secret-bearing file is tracked or on disk. Working tree shows only `.gitignore` + `README.md` modifications. `yarn ci` re-run green: 15 tests passing, build succeeds. No contracts, frontend, deploy scripts, package scripts, env templates, or generated files changed. Repo is safe to push publicly.

**README polish for public visitors (2026-05-05)**
Claude Code · Rewrote `README.md` with a public-demo-ready structure: ASCII banner + flow diagram, plain-English pitch, current status, what works today (with a click-to-call table and a state-machine diagram), what is intentionally deferred, tech stack, a detailed step-by-step Quickstart (prereqs / clone / three-terminal run / first-time wallet / demo / sanity checks), the manual demo flow, useful-docs index, repo layout, safety notes, troubleshooting (10 entries: no ETH, wrong network, missing deployedContracts, past deadline, fee-on-transfer revert, allowance lag, release disabled, lost submissions, lost bounty id, deploy local-only, harmless wallet-stack build warnings, chain restart), roadmap, license. Two ASCII pieces: a PrizeLock banner and a sponsor → escrow → winner flow diagram. Doc-only; no contracts, frontend, deploy scripts, package scripts, env templates, generated files, or tests changed. `yarn ci` green: 15 tests passing, build succeeds.
